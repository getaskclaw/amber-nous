...
New here? Read the [plain-language explainer](../docs/explainers/2026-W39-g47-plain.en.md) first.
...
| token / cost | input **4.29M** / output **0.23M** / cache_read **23.28M** ≈ **$10.7–10.8** (half-price-week rates: $1/M in, $3/M out, $0.25/M cache_read); reconciles with the owner dashboard spend of $10.70 |
| calibration | effort dial live; vision works via data-URI (this endpoint 400s on public-URL images and 1×1 micro-images; the harness embeds base64, unaffected) |
...
2. **Truncation forensics: 0 contested.** All 27 sessions end with cli_close and zero 4xx; the 7 low-token failures (1–2 calls each) were convicted case by case from transcripts as delivery failure, not infrastructure. Method: token forensics only nominates; transcripts convict.
3. **Attribution in the top band.** A-a317e74b at 13/15 (CommandCode same case 14/15; best-in-history band 14/15) — the high-reasoning face is real. Passing still requires all pins, so it counts as a fail.
...
5. **A-d511f9e8 really ran here: 4/12.** The same case went invalid_infrastructure on both CommandCode lanes (1800s cap). This lane proves the case is examinable — the problem sits in the cc lane's infrastructure, not in the case.
...
- Publication rules: transcripts, oracles, and candidate workspaces are never published; the delivery failure is described only by aggregate stats and message counts (2 messages / 0 tool calls), never by quoting question content.

## Verdict

grok-4.7 @ Nous Portal: top-band high-reasoning face (attribution 13/15, build 5/6, ops 5/6, req-drift 4/4, convergence pass), but a hard **agentic delivery-reliability** flaw — 7/24 cases where it talks and does not work, proven as model nature across two providers. **Do not** hand it unattended build or repair jobs. **Do** hand it supervised one-shot Q&A, planning, and attribution analysis. The choice of lane (Nous Portal vs CommandCode) changes quota and price, not this disease.
